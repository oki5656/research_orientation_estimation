...
model = IMUTransformerEncoder(seq_length=seq_length).to(device)
model.train()


imu = torch.rand((seq_length, batch_size, 6)).to(device)
# label = torch.rand(128).to(device)
# model_input = {"imu": imu, "label": label}
res = model(imu)
